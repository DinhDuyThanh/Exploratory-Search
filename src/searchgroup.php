...
$sparql = $prefix."SELECT * WHERE { ?s rdfs:label ?o . FILTER regex(?o, '$tmp','i'). ?s foaf:page ?url} offset ".$offset;
...
while( $row = sparql_fetch_array( $result[$i] ))
{
	...
	$url = str_replace("http://www.freebase.com/view","",$url);
		...
		$name = $row["o"];
		$type = getFreeBaseType($url,$row["s"]);
		$arr_type[$type]="";	
}
